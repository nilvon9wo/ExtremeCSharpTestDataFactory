...
public class VariantResolutionTest
{
    private static readonly ILookupKey Big =
        FlavouredLookupKey.Get(typeof(Account), "reconcile-big").Matching(FieldPredicateFactory.GreaterThan(Field.Of<Account>(nameof(Account.NumberOfEmployees)), 1000));

    private static readonly ILookupKey Small =
        FlavouredLookupKey.Get(typeof(Account), "reconcile-small").Matching(FieldPredicateFactory.LessThan(Field.Of<Account>(nameof(Account.NumberOfEmployees)), 10));
