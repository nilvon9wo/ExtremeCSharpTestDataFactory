...
public class ContactDataProviderTest
{
    ...
    public void SupplyBundle_AtNoneInclusivity_GeneratesTheContactButNotAnAccount()
    {
        ...
        Assert.Null(bundle.GetList(Field.Of<Contact>(x => x.AccountId))); // no Account generated at None inclusivity
    ...
    public void SupplyBundle_AtAllInclusivityInMockMode_GeneratesTheContactAndItsAccountWithTheDocumentedDefaults()
    {
        ...
        // Assert
        AssertContactGenerated(bundle);
        Account generatedAccount = (Account)bundle.GetList(Field.Of<Contact>(x => x.AccountId))![0];
        ...
        Assert.Equal(generatedAccount.Id, ((Contact)bundle.GetList(Field.Of<Contact>(x => x.Id))![0]).AccountId); // the FK is wired
    }
    ...
    public void SupplyBundle_AtAllInclusivityInMockMode_WiresTheGraphWithoutTouchingADatabase()
    {
        ...
        // Assert
        AssertContactGenerated(bundle);
        Assert.NotNull(((Account)bundle.GetList(Field.Of<Contact>(x => x.AccountId))![0]).Id); // the Account got a mock Id
        Assert.Equal(
            ((Account)bundle.GetList(Field.Of<Contact>(x => x.AccountId))![0]).Id,
            ((Contact)bundle.GetList(Field.Of<Contact>(x => x.Id))![0]).AccountId); // mock Ids still wire the FK
    }
    ...
    {
        Contact generated = (Contact)bundle.GetList(Field.Of<Contact>(x => x.Id))![0];
