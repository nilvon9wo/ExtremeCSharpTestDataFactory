...
Every generation operation ultimately produces a `Bundle`. Rather than returning
only the requested records, a Bundle contains the **entire object graph** created
during generation — the requested records, directly related records, indirectly
related records, and the relationships between them.

Bundles make every generated object available without writing extra lookup code.
...
Bundle parentAccountBundle = accountBundle.GetBundle<Account>(x => x.ParentId)!;
object parentAccount = parentAccountBundle.GetList<Account>(x => x.Id)![0];
```
...
(`GetValue(path, rowIndex)`); the one-argument form is row 0. Any hop that was
...
`CopyFromAncestorExpression` is this same walk wrapped as a context-aware value.
...
Bundle? kids               = bundle.GetChildBundle<Contact>(x => x.AccountId);   // navigate on to grandchildren / the children's own parents
```
...
test** needs `contact.Account.Name` or `account.Contacts` off the record itself,
`bundle.Inject(field, config)` / `InjectAll(field)` write those onto new
