# `RecordInjector` — write what `init` rejects

`RecordInjector` is the graft mechanism [`Inject` / `InjectAll`](enrichment.md)
...
`RecordInjector` takes it directly.)
