...
| **Enrichment** — `bundle.Inject(field, config)` / `InjectAll` / `InjectAllParents` / `InjectAllChildren`, `InjectConfig`, standalone `SObjectInjector` | `BundleEnricherTest`, `SObjectInjectorTest`, `EnrichmentSelectionTest`, `EnrichmentIntegrationTest` | [use](../use/enrichment.md), [injector](../use/sobject-injector.md) | ✅ — simpler than Apex (no JSON round-trip, so no Blob/compound/polymorphic special-casing needed) |
