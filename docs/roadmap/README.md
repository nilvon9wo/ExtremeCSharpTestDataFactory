...
| **Classic EF6 persistence** — `Ef6PersistenceGateway`, the same `IPersistenceGateway` convenience as `EfPersistenceGateway` but for `System.Data.Entity.DbContext` (the `EntityFramework` NuGet package, "EF6") rather than `Microsoft.EntityFrameworkCore.DbContext` | `Xfty.EntityFramework6.Test/*` (SQLite), `Ef6SmokeTest` (`Xfty.NetStandardCompat.Test`, `net472`) | [package README](../../Xfty.EntityFramework6/README.md) | ✅ Separate opt-in package (`Xfty.EntityFramework6`) - the two `DbContext` types are unrelated, so this is a distinct implementation, not a variant of the EF Core one. Needed no source changes beyond that: `DbContext.Set(Type).Add(object)`/`SaveChangesAsync()` have been stable since EF6's first release. |
